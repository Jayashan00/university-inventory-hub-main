...
const DashboardLayout: React.FC = () => {
  ...
  // FIX: Move navigation logic to useEffect to prevent "Cannot update during render" error
  useEffect(() => {
    if (!user) {
      navigate('/login');
    }
  ...
  // If user is null, return null (render nothing) while the useEffect redirects
  if (!user) {
    return null;
  }
